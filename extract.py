...
def get_visible_text(_text):
    """Parse raw html obtained from urls and extract only the text that would be visible to a reader visiting
        the websites.  This text is then 'flattenened' (spaces are removed so it is one line)"""
...
def attempt_connection(url, cookie=""):
    ...
    try:
        response = requests.get(url, timeout=15, allow_redirects=True, verify=False, headers=header_dict, stream=True)
